...
rm pubk.tmp* 2>>/dev/null
(
echo -n > pubk.new
curl -Ss http://hia.cjdns.ca/watchlist/walk/walk.pubkey -o pubk.tmp.walk.pubkey
curl -Ss http://hia.cjdns.ca/watchlist/walk/walk.version -o pubk.tmp.walk.version

cat pubk.tmp.walk.pubkey | \
while read PUBK IP
do
#  echo "PUBK:$PUBK IP:$IP"
  ...
  VER="`grep \"$PUBK\" walk.version|cut -d\. -f1|grep ^v`"
  if [ "$VER" != "" ]
  then
