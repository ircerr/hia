...
VPS_IP4="`host mesh.ircerr.ca|grep 'has address'|sed 's/.* has address //g'`"
if [ "VPS_IP4" == "" ]
then
  echo "-ERROR: Unable to resolve vps IPv4"
  exit 1
fi
VPS_IP6="`host mesh.h.ircerr.ca|grep 'has IPv6 address'|sed 's/.* has IPv6 address //g'`"
if [ "VPS_IP6" == "" ]
then
...
#  echo "[$IP]"
  ...
  then
    echo -n "[$IP] ICANN A "
    R="`host mesh.ircerr.ca $IP 2>&1 | grep ' has address '|sed 's/.* has address //g'`"
    if [ "$R" != "" ]
    then
      if [ "$R" == "$VPS_IP4" ]
      then
  ...
  then
    ...
    R="`host mesh.h.ircerr.ca $IP 2>&1 | grep ' has IPv6 address '|sed 's/.* has IPv6 address //g'`"
    if [ "$R" != "" ]
    then
      if [ "$R" == "$VPS_IP6" ]
      then
...
# http://en.wikipedia.org/wiki/Alternative_DNS_root
  if [ "`cat hia-parse-dns.db|grep \"^\[$IP\] YGGTLD \"`" == "" ]
  then
#acetone.ygg has IPv6 address 324:71e:281a:9ed3::ace
    echo -n "[$IP] YGG "
    R="`host acetone.ygg $IP 2>&1 | grep ' has.* address '|sed 's/.* address //g'|sort -n|tr '\n' ' '|sed 's/ $//g'`"
    if [ "$R" != "" ]
    then
      if [ "$R" == "324:71e:281a:9ed3::ace" ]
      then
        echo "OK"
        echo "[$IP] YGGTLD OK" >> hia-parse-dns.db
        if [ -f /tmp/hialog.txt ]
        then
          echo "hia-parse-dns [$IP] YGGTLD OK" >> /tmp/hialog.txt
        fi
      else
        echo "ERR($R)"
        echo "[$IP] YGGTLD ERR($R)" >> hia-parse-dns.db
      fi
    else
      echo "FAIL"
      echo "[$IP] YGGTLD FAIL" >> hia-parse-dns.db
    fi
  fi
