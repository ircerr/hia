...
function cjdnsPing() {
	$CJDNSDIR/tools/cexec "ping()" 2>>/dev/null | grep '"q": "pong"'
}
...
function savekey() {
	...
	NODEIP="$2"
	if [ "$(grep -x "$NODEPK $NODEIP" walk.pubkey 2>>/dev/null)" == "" ]; then
...
function savever() {
	NODEVER=$1
	NODEPK=$2
	if [ "$(grep -x "$NODEVER.$NODEPK" walk.version)" != "" ]; then
		return
	fi
	if [ "$(cat walk.version | cut -d\. -f2 | grep "$NODEPK")" == "" ]; then
...
function addtodo() {
	...
	if [ "$(grep "$NODEPK" tmp/walk/walk.paths.todo)" != "" ]; then
...
function dumpPeers() {
	...
	NODE="$1"
	if [ "$NODE" == "0000.0000.0000.0001" ]; then
		#LocalNode
		NODEPV=""
		NODEPP="$NODE"
		NODEPK=""
	...
	while :; do
		...
		if [ "$(cat tmp/walk/getPeers.$NODE.$PAGE | grep -v '^$')" == "" ] ||
			[ "$(cat tmp/walk/getPeers.$NODE.$PAGE | grep '"result": "timeout"')" != "" ]; then
			RT=$((RT + 1))
			if [ $RT -ge $TRIES ]; then
				if [ "$(cat tmp/walk/getPeers.$NODE.$PAGE | grep '"result": "timeout"')" != "" ]; then
		...
		cat tmp/walk/getPeers.$NODE.$PAGE |
			...
			while read P; do
				...
				if [ "$PP" == "0000.0000.0000.0001" ]; then
					#        echo "--Skipping local node in $P" 1>&2
					continue
				fi
				if [ "$(grep -x "$PV.$PP.$PK.k" tmp/walk/paths.$NODE)" == "" ]; then
...
(
	...
	while :; do
		...
		while [ $BGS -ge $MAXBGS ]; do
			BGS=$(($(ls /tmp/walk/ | grep 'walk.getpeers' | wc -l)))
			if [ $BGS -ge $MAXBGS ]; then
		...
		(
			touch tmp/walk/walk.getpeers.$NEXTPATH
			dumpPeers "$NEXTPATH"
			rm tmp/walk/walk.getpeers.$NEXTPATH
		) & &>/dev/null
		sleep 1
	done
